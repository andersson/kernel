 ...
 * - a region with zero offset and zero size is not defined.
 * - defined offset is *past* all "canary" values
 ...
 *
 * There is a set of filter and routing tables.  Each entry in a filter or
 * route table is IPA_TABLE_ENTRY_SIZE, or 8 bytes.  The number of filter
 * table entries depends on the number of endpoints that support filtering.
 * The first entry in a filter table contains a bitmap, with each set bit
 * indicating an endpoint containing an entry in the table.  Bit 0 is used
 * to represent a global filter.
 *
...
/* The number of routing table entries (IPv4, IPv6; hashed or not) */
#define IPA_MEM_RT_COUNT			15

/* Which routing table entries are for the modem; the rest are for the AP */
