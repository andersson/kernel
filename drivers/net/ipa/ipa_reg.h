 ...
 * is computed based on an "base" offset plus an additional amount based on
 * endpoint's ID and a "stride" value for the register.  For such registers,
 * the offset is computed by a function-like macro that takes a parameter
 ...
 * must be less than IPA_MEM_RT_COUNT.
 *
 ...
 * cases the available field in the ipa structure defines the "full" set of
 * valid bits for the register.
 */

/* Checked when ipa->version is assigned, and it's assumed good thereafter */
static inline bool ipa_reg_version_supported(enum ipa_version version)
{
	switch (version) {
