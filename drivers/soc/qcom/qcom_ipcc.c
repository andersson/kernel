...
static irqreturn_t qcom_ipcc_irq_fn(int irq, void *data)
{
	...
	for (;;) {
		packed_id = __raw_readl(proto_data->base + IPCC_REG_RECV_ID);
...
static void qcom_ipcc_mask_irq(struct irq_data *irqd)
{
	...
	__raw_writel(hwirq, proto_data->base + IPCC_REG_RECV_SIGNAL_DISABLE);
}
...
static void qcom_ipcc_unmask_irq(struct irq_data *irqd)
{
	...
	__raw_writel(hwirq, proto_data->base + IPCC_REG_RECV_SIGNAL_ENABLE);
}
...
static int qcom_ipcc_mbox_send_data(struct mbox_chan *chan, void *data)
{
	...
	__raw_writel(packed_id, proto_data->base + IPCC_REG_SEND_ID);
