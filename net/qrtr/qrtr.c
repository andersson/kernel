 ...
 * @dest_node:	node id of the remote port
 * @dest_port:	port number of the remote port
 */
static void qrtr_tx_resume(struct qrtr_node *node, int dest_node, int dest_port)
{
			...
			int type)
{
	...
	for (;;) {
		ret = wait_event_interruptible(node->resume_tx,
					       flow->pending < QRTR_TX_FLOW_HIGH || !node->ep);
		if (ret)
			return ret;
		...
		mutex_lock(&node->qrtr_tx_lock);
		if (flow->pending < QRTR_TX_FLOW_HIGH) {
			flow->pending++;
...
int qrtr_endpoint_post(struct qrtr_endpoint *ep, const void *data, size_t len)
{
	...
	if (cb->dst_port != QRTR_PORT_CTRL && cb->type != QRTR_TYPE_DATA && cb->type != QRTR_TYPE_RESUME_TX)
		goto err;
...
static void qrtr_node_rx_work(struct work_struct *work)
{
	...
	while ((skb = skb_dequeue(&node->rx_queue)) != NULL) {
		...
		if (cb->type == QRTR_TYPE_RESUME_TX) {
			qrtr_tx_resume(node, cb->src_node, cb->src_port);
			consume_skb(skb);
		} else {
